...
"""Correctness and CUDA-graph tests for direct owner-local DCP FP8 queries."""
...
def _worker(rank: int, world_size: int, port: int) -> None:
    ...
    assert workspace.payload_bytes_per_rank == 1_179_648
    assert workspace.local_query_shard.stride() == (
        query_dim,
        workspace.allocation.bytes_per_rank // local_heads,
        1,
    )
    assert workspace.peer_query.shape == (
        max_rows,
        world_size * local_heads,
        query_dim,
    )
    assert workspace.peer_query.stride() == (
        query_dim,
        workspace.allocation.bytes_per_rank // local_heads,
        1,
    )
    ...
    try:
        for iteration, rows in enumerate((1, 8, 32, 64, 128)):
            ...
            producer = workspace.begin_publish(rows)
            producer.copy_(local_query)
            actual = workspace.acquire_peer_query(rows)
        ...
        producer = workspace.begin_publish(rows)
        producer.copy_(sentinel_bytes.view(torch.float8_e4m3fn))
        actual = workspace.acquire_peer_query(rows)
        torch.accelerator.synchronize()
        ...
        # The installed TRTLLM-GEN sparse MLA binding must consume the global
        # affine-strided peer view directly. Compare both output and LSE
        # bit-for-bit with the same call using a contiguous collective result.
        ...
        producer = workspace.begin_publish(rows)
        producer.copy_(local_query)
        direct_query = workspace.acquire_peer_query(rows)
        ...
        direct_workspace = torch.zeros(
            512 * 1024 * 1024,
            dtype=torch.int8,
            device=device,
        )
        dense_workspace = torch.zeros_like(direct_workspace)
        ...
        direct_output, direct_lse = run_sparse_mla(
            direct_query,
            direct_workspace,
        )
        workspace.acknowledge()
        ...
        with torch.cuda.graph(graph):
            producer = workspace.begin_publish(rows)
            producer.copy_(graph_input)
            graph_output.copy_(workspace.acquire_peer_query(rows))
            workspace.acknowledge()
...
@pytest.mark.skipif(
    ...
    reason="Owner-local DCP query gather requires CUDA.",
)
